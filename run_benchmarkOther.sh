...
OUTPUT_DIR="benchmark/other_${DATA_FILE_NAME}_${MEMORY}MB"  # Dossier pour les résultats
mkdir -p $OUTPUT_DIR
...
for GROUP_SIZE in 1 2 3 6; do
    ...
    java -Xmx${MEMORY}m -jar target/benchmark-other-jar-with-dependencies.jar "$DATA_FILE" "$QUERY_FILE" "$MELANGE" "$GROUP_SIZE"
done

# Fusionner les fichiers CSV en un seul
echo "numberOfQueries,ExecutionTime(ms)" > "${OUTPUT_DIR}/combined_results_other_${DATA_FILE_NAME}_${MEMORY}MB.csv"
for GROUP_SIZE in 1 2 3 6; do
    # Ajouter les résultats de chaque fichier CSV, en sautant la première ligne (en-tête)
    tail -n +2 "${OUTPUT_DIR}/results_other_${DATA_FILE_NAME}_${MEMORY}MB_${GROUP_SIZE}group.csv" >> "${OUTPUT_DIR}/combined_results_other_${DATA_FILE_NAME}_${MEMORY}MB.csv"
done

echo "Fusion des résultats terminée dans ${OUTPUT_DIR}/combined_results_other_${DATA_FILE_NAME}.csv"

# Générer l'histogramme
java -jar target/histogram-jar-with-dependencies.jar "other_${DATA_FILE_NAME}_${MEMORY}MB"

echo "Histogramme généré dans ${OUTPUT_DIR}/histogram_other_${DATA_FILE_NAME}.png"