...
public class BenchmarkIntegralRunner {

    public static void main(String[] args) throws IOException {
        if (args.length < 4) {
            System.err.println("Usage: java -jar benchmark-integral.jar <fichier_triplet> <fichier_requete> <melange> <group_size>");
            System.exit(1);
        }
        ...
        File benchmarkDir = new File("benchmark/integral_" + dataFileName + "_" + memory + "MB");
        if (!benchmarkDir.exists()) {
            benchmarkDir.mkdirs(); // Create the directory if it doesn't exist
        }
        String outputFile = "benchmark/integral_" + dataFileName + "_" + memory + "MB/results_integral_" + dataFileName + "_" + memory + "MB_" + groupSize + "group";
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(txtFile));
            ...
            runBenchmarks(factBase, queryGroups, writer, csvWriter, executionTimes);
            writer.write("\n");
    ...
    private static void runBenchmarks(FactBase factBase, List<List<StarQuery>> queryGroups, BufferedWriter writer, BufferedWriter csvWriter, List<Long> executionTimes) throws IOException {
        ...
        for (List<StarQuery> group : queryGroups) {
            ...
            for (StarQuery query : group) {
                executeQueryWithIntegral(query, factBase);
            }
    ...
    private static void executeQueryWithIntegral(StarQuery starQuery, FactBase factBase) {
        ...
        Iterator<Substitution> results = evaluator.evaluate(foQuery, factBase);
