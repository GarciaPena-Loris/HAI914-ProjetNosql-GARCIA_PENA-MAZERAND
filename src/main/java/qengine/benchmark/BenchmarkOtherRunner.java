...
import qengine.otherStorage.RDFHexaStore;
...
public class BenchmarkOtherRunner {

    public static void main(String[] args) throws IOException {
        if (args.length < 4) {
            System.err.println("Usage: java -jar benchmark-other.jar <fichier_triplet> <fichier_requete> <melange> <group_size>");
            System.exit(1);
        }
        ...
        File benchmarkDir = new File("benchmark/other_" + dataFileName + "_" + memory + "MB");
        if (!benchmarkDir.exists()) {
            benchmarkDir.mkdirs(); // Create the directory if it doesn't exist
        }
        String outputFile = "benchmark/other_" + dataFileName + "_" + memory + "MB/results_other_" + dataFileName + "_" + memory + "MB_" + groupSize + "group";
